...
export function Tasklist() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        // Fetch rentals
        const rentalsResponse = await fetch("http://localhost:4000/api/rentas/completadas")
        const rentalsData = await rentalsResponse.json()


        // Fetch ratings
        const ratingsResponse = await fetch("http://localhost:4000/api/rentas/calificaciones")
  ...
  async function handleGuardar() {
    ...
    try {
      ...
      const ratingData = {
        ...
        id_calificador: 4,
        id_calificado: selected.idReserva,
      }
      ...
      const url = existingRating
        ? `http://localhost:4000/api/rentas/calificaciones/${existingRating.idCalificacion}`
        : "http://localhost:4000/api/rentas/calificaciones"
  ...
  async function handleBorrar(renter: Renter) {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:4000/api/rentas/calificaciones/${calificacion.idCalificacion}`, {
        method: "DELETE",
      })
      if (!response.ok) {
