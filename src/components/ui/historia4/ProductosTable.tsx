  ...
  const fetchReservations = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/host-history?hostId=1&page=${currentPage}&limit=${itemsPerPage}`
      );
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {sortedNestedReservations.map((reserva) => (
              <tr key={`${reserva.marca}-${reserva.modelo}-${reserva.fecha_inicio}`} className="hover:bg-gray-50">
